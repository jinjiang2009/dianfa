<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
 class login_model extends CI_Model{
	...
	/**
 	 ...
 	 */
 	function exc($id){
 		#192.168.0.251 database
 		$this->_db = $this->load->database('df',true);
 		$rs = $this->_db->get_where('test423zhishig',array('id'=>$id),1,0)->result_array();
 		return $rs;
 		#192.168.0.248 database
 		// return $this->db->get_where('test423min',array('id'=>$id),1,0)->result_array();
 	}
...
//      return $this->db->get_where('df_court',array('name'=>$name),1,0)->result_array();
 	...
 	function addju($p){
 		$this->db->insert('df_judgement_copy423',$p);
 		return $this->db->insert_id();
 	}
 	...
 	public function getCity($id){
 		$sql = ' select * from df_region where type=2 and pid= "'.$id.'"';
 		$result = $this->db->query($sql)->result_array();
 		return $result;
 	}
 	public function getCountry($id){
 		$sql = ' select * from df_region where type=3 and pid= "'.$id.'"';
 		$result = $this->db->query($sql)->result_array();
 		return $result;
 	}
 }
