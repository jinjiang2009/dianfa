...
class lhome extends base{
  ...
  public function index(){
  	...
  	if($_FILES){
  		...
  		$this->home->updateLawyer('df_job_reply',$data['user']['ucode'],array('icon'=>$filename));//更新用户信息
  		if(empty($data['user']['icon'])){//var_dump($data['user']['icon']);exit;
  	...
  	if(isset($post['all'])&&$post['all']=='mod'){
  		$la = $this->home->getLawyer($data['user']['ucode']);//获得律师的信息
  		...
  		if(empty($la['preyear'])&&!empty($post['preyear'])){
  			$in += 10;
  			$aa = true;
  		}
  		...
  		if(empty($la['work'])&&!empty($post['work'])){
  			$in += 10;
  			$aa = true;
  		}
  		if(empty($la['secre'])&&!empty($post['secre'])){
  			$in += 10;
  			$aa = true;
  		}
  		...
  		if(empty($la['court'])&&!empty($post['court'])){
  			$in += 10;
  			$aa = true;
  		}
  		...
  		$arr = array(
  				'name'=>str_replace('律师','',$post['realname']),//律师真实姓名
  				'region'=>$post['region'],//所在城市
  				'sex'=>$post['sex'],//性别
  				'preyear'=>$post['preyear'],//执业年限
  				'license'=>$post['license'],//律师执业证号
  				'work'=>$post['work'],//执业机构
  				'court'=>$post['court'],//常去法院
  				'rerritory'=>join(',',$post['rerritory']),//擅长领域
  				'secre'=>join(',',$post['secre']),//二级擅长领域
  				'descript'=>$post['descript'],//介绍自己
  				);
  		$this->home->updateLawyer('df_lawyer',$data['user']['ucode'],$arr);
  	}
  	...
  	$data['lawyer'] = $this->home->getLawyer($data['user']['ucode']);//获得律师的信息
  	//p($data['lawyer']);die;
  	$data['user'] = $this->session->userdata('user');
  	$this->load->view('default/lhome_info.php',$data);
  }
  /*
   * 我的文档
  */
  ...
  public function article(){
  	...
  	if(!empty($post)){
  		...
  		$this->home->addOne('df_lawyer','contract',$data['user']['ucode']);
  		$this->integral_change(array('ucode'=>$data['user']['ucode'],'cause'=>13,'cor_id'=>'','num'=>10,'type'=>1));
  		$data['user']['integral'] = $data['user']['integral']+10;
  		$this->session->set_userdata ( 'user', $data['user'] );
  	}
  	$this->load->view('default/lhome_article.php',$data);
  }
  /*
   * 异步获取分类
   */
  ...
  */
  public function upjudge(){
  	...
  	if(!empty($post)){
  		...
  		$this->home->addOne('df_lawyer','remark',$data['user']['ucode']);
  		$this->integral_change(array('ucode'=>$data['user']['ucode'],'cause'=>13,'cor_id'=>'','num'=>10,'type'=>1));
  		$data['user']['integral'] = $data['user']['integral']+10;
  		$this->session->set_userdata ( 'user', $data['user'] );
  	}
  	$this->load->view('default/lhome_judgement.php',$data);
  }
  /*
   * 修改密码
   */
  ...
  */
  ...
  public function identity(){
  	...
  	if($_FILES){
  		...
  		if($type==1){
  			...
  			$filename = $rs['file_name'];
  			$law = $this->home->getLawyer($data['user']['ucode']);
  			$this->home->updateLawyer('df_lawyer',$data['user']['ucode'],array('icon'=>$filename));
  			$user = $this->session->userdata('user');
  			$user['lawyericon'] = $filename;
  			if(empty($law['icon'])){
  			...
  			$filename = $rs['file_name'];
  			$law = $this->home->getLawyer($data['user']['ucode']);
  			$this->home->updateLawyer('df_lawyer',$data['user']['ucode'],array('lince_img'=>$filename));
  			$user = $this->session->userdata('user');
  	...
  	$data['user'] = $this->session->userdata('user');
  	if(!isset($data['user']['lawyericon'])||!isset($data['user']['linceimg'])){
  		
  		$te =  $this->home->getLayerIcon($data['user']['ucode']);
  		$data['user']['lawyericon'] = $te['icon'];
  		$data['user']['linceimg'] = $te['lince_img'];
  	}
  	$this->load->view('default/lhome_identity.php',$data);
  }
  ...
  public function integral(){
  	...
  	$data['totalpage'] = ceil($data['total']/$perpage);
  	$config = $this->page_config($data['total'],$perpage,STATIC_URL.'lhome/integral?');
  	$this->pagination->initialize($config);
  	$data['page'] = $this->pagination->create_links();
  	$data['inte'] = $this->integral_config();
  	$this->load->view('default/lhome_integral.php',$data);
  }
  /*
   * 我的咨询
  */
  public function ask(){
  	...
  	$data['totalpage'] = ceil($data['total']/$perpage);
  	$config = $this->page_config($data['total'],$perpage,STATIC_URL.'lhome/ask?');
  	$this->pagination->initialize($config);
  	$data['page'] = $this->pagination->create_links();
  	$this->load->view('default/lhome_ask.php',$data);
  }
  /*
   * 系统消息
  */
  public function message(){
  	...
  	$data['totalpage'] = ceil($data['total']/$perpage);
  	$config = $this->page_config($data['total'],$perpage,STATIC_URL.'lhome/message?');
  	$this->pagination->initialize($config);
  	$data['page'] = $this->pagination->create_links();
  	$this->load->view('default/lhome_message.php',$data);
  }
  /*
   * 系统消息详情
  */
  ...
  */
  public function order(){
  	...
  	$data['totalpage'] = ceil($data['total']/$perpage);
  	$config = $this->page_config($data['total'],$perpage,STATIC_URL.'lhome/order?');
  	$this->pagination->initialize($config);
  	$data['page'] = $this->pagination->create_links();
  	$data['type'] = array(1=>'合同文书',2=>'咨询',3=>'产品服务');
  	$this->load->view('default/lhome_order.php',$data);
  }
  /*
   * 我投过标的案件
   */
  public function cases(){
  	...
  	$data['totalpage'] = ceil($data['total']/$perpage);
  	$config = $this->page_config($data['total'],$perpage,STATIC_URL.'lhome/cases?');
  	$this->pagination->initialize($config);
  	$data['page'] = $this->pagination->create_links();
  	$this->load->view('default/lhome_case.php',$data);
  }
