...
class SaeTOAuthV2 {
	...
	switch ($method) {
		...
		default:
			...
			fwrite($handle, $body);
			fwrite($handle, var_export($headers,true));
			fclose($handle);
	...
	function http($url, $method, $postfields = NULL, $headers = array()) {
		...
		$handle = fopen('./log.txt', 'a+');
		fwrite($handle, var_export($postfields,true));
		fwrite($handle, var_export(curl_getinfo($ci),true));
		fwrite($handle, var_export($headers,true));
		...
		fwrite($handle, curl_errno($ci));
		fwrite($handle, curl_error($ci));
		fwrite($handle, var_export($this->http_info,true));
		fclose($handle);
		if ($this->debug) {
			echo "=====post data======\r\n";
			var_dump($postfields);
